import React, { useState, useEffect } from "react";
import { useForm } from "react-hook-form";
import { motion } from "framer-motion";
import { Eye, EyeOff, ArrowUp, LogIn, Loader2 } from "lucide-react";
import { Link } from "react-router-dom";
import toast, { Toaster } from "react-hot-toast";

const SignupPage = () => {
  const { register, handleSubmit, formState: { errors } } = useForm();
  const [showPassword, setShowPassword] = useState(false);
  const [isLoadingLogin, setIsLoadingLogin] = useState(false);
  const [isLoadingGoogle, setIsLoadingGoogle] = useState(false);

  // Typewriter state
  const statements = [
    "Unlock legal insights with AI...",
    "Streamline your research process...",
    "Access case law instantly...",
    "Empower your practice today..."
  ];
  const [statementIndex, setStatementIndex] = useState(0);
  const [displayedText, setDisplayedText] = useState("");
  const [isTyping, setIsTyping] = useState(true);
  const [cursorVisible, setCursorVisible] = useState(true);

  // Cursor blinking
  useEffect(() => {
    const blink = setInterval(() => {
      setCursorVisible((prev) => !prev);
    }, 500);
    return () => clearInterval(blink);
  }, []);

  // Typing + erasing logic
  useEffect(() => {
    let typingTimeout, eraseTimeout, nextStatementTimeout;
    const current = statements[statementIndex];

    if (isTyping) {
      if (displayedText.length < current.length) {
        typingTimeout = setTimeout(() => {
          setDisplayedText(current.slice(0, displayedText.length + 1));
        }, 80); // typing speed
      } else {
        nextStatementTimeout = setTimeout(() => setIsTyping(false), 2000); // pause before erase
      }
    } else {
      if (displayedText.length > 0) {
        eraseTimeout = setTimeout(() => {
          setDisplayedText(displayedText.slice(0, -1));
        }, 40); // erasing speed
      } else {
        nextStatementTimeout = setTimeout(() => {
          setStatementIndex((prev) => (prev + 1) % statements.length);
          setIsTyping(true);
        }, 500);
      }
    }

    return () => {
      clearTimeout(typingTimeout);
      clearTimeout(eraseTimeout);
      clearTimeout(nextStatementTimeout);
    };
  }, [displayedText, isTyping, statementIndex]);

  const onSubmit = async (data) => {
    setIsLoadingLogin(true);
    try {
      await new Promise((resolve) => setTimeout(resolve, 2000));
      if (data.email && data.password) {
        toast.success("Login successful!", {
          style: glassyToastStyle,
        });
      } else {
        throw new Error("Invalid credentials");
      }
    } catch (error) {
      toast.error("Login failed: " + error.message, {
        style: glassyToastStyle,
      });
    } finally {
      setIsLoadingLogin(false);
    }
  };

  const handleGoogleLogin = async () => {
    setIsLoadingGoogle(true);
    try {
      await new Promise((resolve) => setTimeout(resolve, 2000));
      toast.success("Signed in with Google!", { style: glassyToastStyle });
    } catch {
      toast.error("Google sign-in failed!", { style: glassyToastStyle });
    } finally {
      setIsLoadingGoogle(false);
    }
  };

  // Show toast for validation errors
  useEffect(() => {
    if (errors.email || errors.password) {
      const msg = errors.email?.message || errors.password?.message || "Please fix the errors above";
      toast.error(msg, { style: glassyToastStyle });
    }
  }, [errors]);

  return (
    <div className="min-h-screen flex flex-col lg:flex-row overflow-hidden">
      <Toaster position="top-right" />

      {/* Left Side: Form */}
      <div className="lg:w-1/2 w-full min-h-screen bg-black flex items-center justify-center p-4 sm:p-8">
        <motion.form
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          onSubmit={handleSubmit(onSubmit)}
          className="w-full max-w-md space-y-6"
        >
          <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mt-25">
            Sign up for an account
          </h2>

          {/* Email */}
          <div className="space-y-2">
            <label className="text-white text-sm font-medium">Full name</label>
            <input
              {...register("text", { required: "Name is required", pattern: { value: /^\S+@\S+$/i, message: "Invalid name" } })}
              placeholder="Enter Name..."
              className="w-full p-3 backdrop-blur-xl bg-white/10 text-white placeholder-white/60 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 mt-3"
            />
          </div>
          {/* Email */}
          <div className="space-y-2">
            <label className="text-white text-sm font-medium">Email address</label>
            <input
              {...register("email", { required: "Email is required", pattern: { value: /^\S+@\S+$/i, message: "Invalid email" } })}
              placeholder="email@you.com"
              className="w-full p-3 backdrop-blur-xl bg-white/10 text-white placeholder-white/60 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 mt-3"
            />
          </div>

          {/* Password */}
          <div className="space-y-2">
            <label className="text-white text-sm font-medium">Password</label>
            <div className="relative">
              <input
                {...register("password", { required: "Password is required", minLength: { value: 6, message: "Min 6 characters" } })}
                type={showPassword ? "text" : "password"}
                placeholder="......."
                className="w-full p-3 bg-white/10 backdrop-blur-xl text-white placeholder-white/60 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-white/20 mt-3"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-1/2 transform -translate-y-1/2 text-white/60 hover:text-white transition"
              >
                {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
              </button>
            </div>
          </div>

          

          {/* Login Button */}
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            type="submit"
            disabled={isLoadingLogin}
            className={`w-full py-3 bg-white/20 text-white rounded-lg font-medium hover:bg-white/40 transition flex items-center justify-center gap-2 cursor-pointer ${
              isLoadingLogin ? "opacity-70 cursor-not-allowed" : ""
            }`}
          >
            {isLoadingLogin ? <Loader2 size={18} className="animate-spin" /> : <LogIn size={18} />}
            Sign up
          </motion.button>

          <p className="text-center text-white/80 text-sm">
           Already have an account?{" "}
            <Link to="/login" className="font-bold text-white hover:text-white/40">
             Sign in
            </Link>
          </p>

          {/* Divider */}
          <div className="flex items-center gap-3">
            <div className="flex-grow h-px bg-white/20"></div>
            <span className="text-white/60 text-sm">or continue with</span>
            <div className="flex-grow h-px bg-white/20"></div>
          </div>

          {/* Google Button */}
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            type="button"
            onClick={handleGoogleLogin}
            disabled={isLoadingGoogle}
            className={`w-full py-3 bg-white/10 backdrop-blur-lg text-white rounded-lg font-medium hover:bg-white/20 transition flex items-center justify-center gap-2 cursor-pointer ${
              isLoadingGoogle ? "opacity-70 cursor-not-allowed" : ""
            }`}
          >
            {isLoadingGoogle ? <Loader2 size={18} className="animate-spin" /> : <img src="https://www.google.com/favicon.ico" alt="Google Logo" className="w-5 h-5" />}
            Continue with Google
          </motion.button>
        </motion.form>
      </div>

      {/* Right Side: Hero */}
      <div className="lg:w-1/2 hidden lg:flex relative bg-cover bg-center" style={{ backgroundImage: "url('/images/hero-bg.jpg')" }}>
        <div className="absolute inset-0 bg-black/40"></div>
        <div className="relative z-10 flex flex-col items-center justify-center w-full h-full p-8">
          {/* Typewriter box */}
          <div className="w-full max-w-lg">
            <div className="p-4 bg-white text-black text-lg rounded-lg shadow-md">
              {displayedText}
              <span
                className={`inline-block w-[2px] bg-black ml-0.5 ${
                  cursorVisible ? "opacity-100" : "opacity-0"
                } transition-opacity duration-200`}
              >
                &nbsp;
              </span>
            </div>
          </div>

          {/* Round button */}
          <div className="relative bottom-12 left-50">
            <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center shadow-lg">
              <ArrowUp size={24} className="text-white" />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

const glassyToastStyle = {
  background: "rgba(255, 255, 255, 0.1)",
  backdropFilter: "blur(8px)",
  color: "white",
  border: "1px solid rgba(255, 255, 255, 0.2)",
  borderRadius: "8px",
};

export default SignupPage;
